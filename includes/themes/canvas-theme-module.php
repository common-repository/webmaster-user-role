...
class SCD_Canvas_Theme {
	...
	function is_active() {
		...
		if ( $current_theme->Name == 'canvas' || $current_theme->Template == 'canvas' ) {
			$this->active = true;
	...
	function setting_fields( $fields = array() ) {
		...
		$fields   = array();
		$fields[] = array(
			'id'       => 'canvas_theme_settings',
			'type'     => 'checkbox',
			'title'    => __( 'Canvas Theme Compatibility', 'webmaster-user-role' ),
			'subtitle' => __( 'Webmaster (Admin) users can', 'webmaster-user-role' ),
			'options'  => array(
				'access_theme_options_panel' => __( 'Access Canvas Theme Options', 'webmaster-user-role' ),
			),
	...
	function admin_menu() {
		...
		if ( ! empty( $webmaster_user_role_config['canvas_theme_settings']['access_theme_options_panel'] ) ) {
			return;
		}
