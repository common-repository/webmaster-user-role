...
class SCD_Boss_Theme {
	...
	function is_active() {
		...
		if ( $current_theme->Name == 'boss' || $current_theme->Template == 'boss' ) {
			$this->active = true;
	...
	function setting_fields( $fields = array() ) {
		...
		$fields   = array();
		$fields[] = array(
			'id'       => 'boss_theme_settings',
			'type'     => 'checkbox',
			'title'    => __( 'Boss Theme Compatibility', 'webmaster-user-role' ),
	...
	function admin_menu() {
		...
		if ( empty( $webmaster_user_role_config['boss_theme_settings']['access_theme_options_panel'] ) ) {
